...
class qn (np.ndarray) :
    """Structured `numpy.ndarray` subclass representing an array of quaternions.

    Each element stores the four quaternion components as named fields
    `w`, `x`, `y`, `z` (real, i, j, k) via `qn_dtype`, so a `qn` instance
    supports the same indexing/broadcasting/reshaping as `numpy.ndarray`
    while its arithmetic operators follow quaternion algebra.
    """
    ...
    def __getitem__(self, keys):
        ...
        """
        if keys in ['w', 'x', 'y', 'z']:
            arr_self = self.view(np.ndarray).view(self.qn_dtype).__getitem__(keys)
            return arr_self.view(np.ndarray)
        else:
            # if type(keys) == int:  # If index is integer, converted to slices; otherwise cause error
            #     keys = slice(keys, keys + 1)
            arr_self = self.matrixform.__getitem__(keys)
            return arr_self.view(qn)

    def __iter__(self):
        """Return self as the iterator; iteration state is tracked in `_iter_counter`."""
        return self

    def __next__(self):
        """Return the next element along axis 0, raising StopIteration (and resetting the counter) when `__getitem__` raises IndexError."""
        try:
    ...
    def __neg__(self):
        """Elementwise negation: -q (each of w, x, y, z multiplied by -1)."""
    ...
    def __add__(self, qn2):
        """Componentwise addition: qn1 + qn2, or qn1 + r with r a real/ndarray scalar broadcast onto all four components."""
        # Elementary arithmetic: qn1 + qn2 or qn1 + r (real number). Same as the elementary arithmetic for real number
        if any([1 if (qn2.__class__ == k) else 0 for k in (int, float, np.ndarray, np.float64, np.float32)]):
    ...
    def __iadd__(self, qn2):
        """In-place addition; delegates to `__add__`."""
        # Elementary arithmetic: qn1 += qn2 or qn1 += r
        return self.__add__(qn2)

    def __radd__(self, qn2):
        """Reflected addition: r + qn1; delegates to `__add__` since quaternion addition is commutative."""
        # Elementary arithmetic: qn2 + qn1 or r + qn1
        return self.__add__(qn2)

    def __sub__(self, qn2):
        """Componentwise subtraction: qn1 - qn2, or qn1 - r with r a real/ndarray scalar broadcast onto all four components."""
    ...
    def __isub__(self, qn2):
        """In-place subtraction; delegates to `__sub__`."""
        # Elementary arithmetic: qn1 -= qn2 or qn1 -= r
        return self.__sub__(qn2)

    def __rsub__(self, qn2):
        """Reflected subtraction: r - qn1 (or qn2 - qn1), computed directly since subtraction is not commutative."""
        # Elementary arithmetic: qn2 - qn1 or r - qn1
        if any([1 if (qn2.__class__ == k) else 0 for k in (int, float, np.ndarray, np.float64, np.float32)]):
    ...
    def __mul__(self, qn2):
        """Hamilton product: qn1 * qn2, or elementwise scaling by a real/ndarray scalar.
    ...
    def __rmul__(self, qn2):
        """Reflected Hamilton product: qn2 * qn1.

        Differs from `__mul__`'s result because quaternion multiplication
        is non-commutative; scalar multiplication still commutes.
        """
    ...
    def __imul__(self, qn2):
        """In-place Hamilton product; delegates to `__mul__`, so `qn1 *= qn2` means `qn1 = qn1 * qn2`, not `qn2 * qn1`."""
        # Elementary arithmetic: qn1 *= qn2; check https://en.wikipedia.org/wiki/Quaternion#Algebraic_properties for
        # details
        return self.__mul__(qn2)

    def __truediv__(self, qn2):
        """qn1 / qn2, computed as qn1 * qn2.inv for quaternion operands, or elementwise division for a real/ndarray scalar.

        Not commutative with `__rtruediv__`.
        """
        # Elementary arithmetic: qn1 / qn2; Note the result of __truediv__ and __rtruediv__ are not equal for quaternion
        if any([1 if (qn2.__class__ == k) else 0 for k in (int, float, np.float64, np.float32)]):
            compactProduct = self.matrixform / qn2
            compactProduct = np.reshape(compactProduct.view(self.qn_dtype), compactProduct.shape[:-1])
    ...
    def matrixform(self):
        ...
        if self.itemsize == 8:
            return self.view(np.ndarray).view((np.double,1))
        else:
            return self.view(np.ndarray).view((np.double,4))
    ...
    @property
    def imagpart(self):
        """Imaginary part as a plain (M x ... x 3) ndarray of the (x, y, z) components (not a `qn`)."""
        # Return the double real number matrix (M x ... x 3) of the imaginary part
        return self.matrixform[..., 1:]

    @property
    def realpart(self):
        """Real part as a plain ndarray of the `w` component (not a `qn`)."""
        # Return the double real number matrix (M x ... x1) of the real part
        return self['w']

    @property
    def norm(self):
        """Euclidean norm (magnitude) of each quaternion: sqrt(w**2 + x**2 + y**2 + z**2)."""
        # Return the norm (or the absolute value) of the quaternion number
        return np.sqrt(np.sum(self.matrixform ** 2, axis=-1))

    @property
    def normalize(self):
        """Unit-norm quaternion(s): self divided elementwise by `norm`."""
        # Return the normalized quaternion number (norm = 1)
        return self / self.norm

    @property
    def leftmul_matrix(self):
        """4x4 matrix representation of left-multiplication by this quaternion.

        I.e. the matrix L such that `L @ q2.compact == (self * q2).compact`.
    ...
    def conj_sandwich_mat(self):
        """4x4 matrix QN such that `QN @ q2.compact == (self * q2 * self.conj).compact`; equivalent to this quaternion's rotation matrix."""
    ...
    def sandwich_mat(self):
        """4x4 matrix QN such that `QN @ q2.compact == (self * q2 * self).compact` (no conjugation); equivalent to this quaternion's reflection matrix."""
        # Return the corresponding matrix QN so (qn1*qn2*qn).matrixform = QN*[qn2.w;qn2.x,qn2.y;qn2.z]
        # Equal to reflection matrix
        return sliceDot(-self.leftmul_matrix, self.leftmul_matrix)

    def sum(self, **kwargs):
        """Sum this array along `axis` (default 0), converting through `matrixform`.

        Kept for `ndarray` API compatibility; prefer the module-level
        `qnum.sum` function instead.
        """
    ...
    @property
    def up(self):
        """Projection of self onto the qn(0,0,0,1) axis, normalized; the "up" direction under this quaternion's orientation."""
        # computed the projection on the axis qn(0,0,0,1) as the up vector
        return projection(self, qn([0, 0, 0, 1])).normalize

    @property
    def down(self):
        """Projection of self onto the qn(0,0,0,-1) axis, normalized; the "down" direction under this quaternion's orientation."""
        # computed the projection on the axis qn(0,0,0,-1) as the down vector
        return projection(self, qn([0, 0, 0, -1])).normalize

    @property
    def left(self):
        """Cross product of self with `up`, normalized.

        Note: the source comment for this property describes it as the
        "right" vector, and `right` below is commented as the "left"
        vector -- the left/right naming is inconsistent between the
        property names and the original comments, and was left as found.
        """
        # computed the projection on the axis qn(0,0,1,0) as the right vector
        return qcross(self,self.up).normalize

    @property
    def right(self):
        """Cross product of self with `down`, normalized.

        See the note on `left` above regarding the swapped left/right
        naming in the original comments.
        """
        # computed the projection on the axis qn(0,0,-1,0) as the left vector
        return qcross(self,self.down).normalize

    def component_at(self, point):
        """Velocity vector at `point` induced by treating self's imaginary part as an infinitesimal rotation about the origin.

        `point` must be a `qn`; it is normalized internally.
        """
        # Return the velocity of the rotation the quaternion represents at the given point
        # The point should be a quaternion number
        return -qcross(self.imag, point.normalize)

    def speed_at(self, point, velocity_direction):
        """Signed scalar speed at `point` along `velocity_direction`.

        Projects the rotational velocity from `component_at` onto
        `velocity_direction` (not onto its tangent plane) and signs the
        magnitude by the sign of `qdot` between the projection and
        `velocity_direction`.
        """
        qn_vel = self.component_at(point)
        proj = projection(velocity_direction.normalize, qn_vel, on_plane=False)
        proj_sign = np.sign(qdot(proj, velocity_direction))
        return proj_sign*proj.norm

    @property
    def w(self):
        """Real (`w`) component, as a plain ndarray."""
        # Return the real part of the quaternion number
        return self['w']

    @property
    def x(self):
        """`x` (i) component, as a plain ndarray."""
        # Return the x part of the quaternion number
        return self['x']

    @property
    def y(self):
        """`y` (j) component, as a plain ndarray."""
        # Return the y part of the quaternion number
        return self['y']

    @property
    def z(self):
        """`z` (k) component, as a plain ndarray."""
        # Return the z part of the quaternion number
        return self['z']

    @property
    def angle(self):
        """Rotation angle in radians, 2*arccos(w).

        Assumes self is a unit (normalized) rotation quaternion; result
        is in [0, 2*pi].
        """
        # Return the angle of the quaternion number
        return 2*np.arccos(self['w'])

    @property
    def axis(self):
        """Rotation axis: the normalized imaginary part.

        Assumes self is a unit rotation quaternion.
        """
        # Return the axis of the quaternion number
        return self.imag.normalize

################################### Functions ###################################
def compute_rotation_from_motions(motion_qn_arr, location_qn_arr):
    """Recover a rotation axis/angle from motion vectors sampled at points on a unit sphere.

    `motion_qn_arr` and `location_qn_arr` must be the same length; only
    the first two samples are used (a warning is issued if more are
    given, and separately if `location_qn_arr` is not unit-normalized).
    The returned angle is clipped to [-2*pi, 2*pi].
    """
    # Compute the rotation quaternion from the motion vector in quaternion form measured at the locations on an unit
    # sphere defined by the location quaternion array, the rotation speed computed is in the range of [0, pi]
    if any(location_qn_arr.norm != 1):
        warnings.warn('The location quaternion array is not normalized, the result may not be correct')
    assert len(motion_qn_arr) == len(location_qn_arr), 'The length of motion and location array should be the same'

    if len(motion_qn_arr) > 2:
        warnings.warn('The length of motion and location array is larger than 2, the rotation vector will only be calculated from the first 2 points')
    ...
    rot_angle = np.clip(np.nanmean(motion_qn_arr.matrixform/rec_rotation_axis.component_at(location_qn_arr).matrixform), -np.pi*2, np.pi*2)
    return rec_rotation_axis, float(rot_angle)


def difference(qn1, qn2): # Check if two quaternion numbers are parallel
    """Complex value exp(i*anglebtw(qn1, qn2)) * (qn1.norm/qn2.norm), combining their angle and norm ratio.

    Despite the trailing comment, this does not itself return a
    boolean "are they parallel" check -- the angle is recoverable from
    the phase and the norm ratio from the modulus, but how a caller is
    meant to interpret/threshold the combined complex value is not
    documented elsewhere in this file.
    """
    return np.exp(1j*anglebtw(qn1, qn2))*(qn1.norm/qn2.norm)
...
def stack(*qn_array, **kwargs):
    """Stack multiple `qn` arrays into one array of quaternions.

    `axis=0` uses `np.hstack`, `axis=1` uses `np.vstack`, and any other
    `axis` (including unset) falls through to `np.stack`; this
    axis-to-function mapping is taken as-is from the original code and
    its rationale (it does not match `np.stack`'s own axis convention)
    was not verified against call sites.
    """
    # Same as np.stack
...
def sum(*qn_array, **kwargs):
    """Sum multiple `qn` arrays elementwise along `axis` (default 0), mirroring `numpy.sum` over a sequence of quaternions.

    Unlike the `qn.sum` instance method (which reduces one array along
    an axis), this takes multiple `qn` arrays as positional arguments,
    stacks them, and sums across that stack.
    """
    # Same as np.sum
    sum_axis = kwargs.pop('axis', None)
    if sum_axis:
...
def nansum(*qn_array, **kwargs):
    """Like the module-level `sum`, but combines the stacked `qn` arrays with `numpy.nansum` so NaNs are ignored."""
...
def mean(*qn_array, **kwargs):
    """Elementwise mean of multiple `qn` arrays along `axis` (default 0), mirroring `numpy.mean` over a sequence of quaternions."""
    # Same as np.mean
    sum_axis = kwargs.pop('axis', None)
    if sum_axis:
        if sum_axis < 0:
...
def exp(qn1):
    """Quaternion exponential: exp(w + v) = e**w * (cos(|v|) + v/|v| * sin(|v|)), the standard quaternion exp map.

    Note that `qn1**2` / general integer power is still not implemented
    in this module.
    """
...
# def log(qn1):
#     # Logarithm calculation for quaternion numbers
#     coeff_real = np.log(qn1.realpart)
#     coeff_imag_norm = qn1.imag.norm
#     coeff_imag = np.arccos(qn1.realpart/np.abs(coeff_real)) * coeff_imag_norm
#     temp_shape = qn1['w'].shape
#     if not temp_shape: # If the input is a single quaternion number
#         compactProduct = np.zeros([qn1['w'].size], dtype=qn1.qn_dtype)
#     else:
#         compactProduct = np.zeros([*temp_shape], dtype=qn1.qn_dtype)
#     compactProduct = np.full_like(compactProduct, np.nan)
#     compactProduct['w'] = coeff_real
#     compactProduct['x'] = qn1['x'] * coeff_imag
#     compactProduct['y'] = qn1['y'] * coeff_imag
#     compactProduct['z'] = qn1['z'] * coeff_imag
#     return compactProduct.view(qn)
...
#
def qdot(qn1, qn2):
    """Dot product of the vector parts of two quaternions: -(qn1 * qn2).realpart.

    Matches the 3D vector dot product only when qn1 and qn2 are pure
    (real part 0); for general quaternions it is simply the negated
    real part of the Hamilton product.
    """
...
def qcross(qn1, qn2):
    """Cross product of the vector parts of two quaternions: (qn1 * qn2).imag.

    Matches the 3D vector cross product only when qn1 and qn2 are pure
    (real part 0); for general quaternions it is simply the imaginary
    part of the Hamilton product.
    """
...
def anglebtw(qn1, qn2):
    """Angle in [0, pi] between two 3D vectors represented as pure quaternions, via the chord-length identity |a-b| = 2*sin(theta/2).

    Both inputs are normalized internally before the angle is computed.
    A slower `arccos`-based equivalent is left commented out below.
    """
...
def rotation_qn(rot_axis, rot_angle=None):
    """
    Calculate the quaternion number for the rotation transformation
    ------------
    :param rot_axis:  rotation axis (1 x 3 ndarray)
    :param rot_angle: optional, real number defines the rotation angle
    :return:  quaternion number for the corresponding rotation
    """
    axistype = type(rot_axis)
    if axistype == np.ndarray or axistype == list:
        rot_axis = qn(rot_axis)
    elif axistype == qn:
        pass
    else:
        raise Exception('Rotation axis should be a ndarray or a quaternion, instead its type is %s\n' % axistype)
    if rot_angle is not None:
        rot_axis = np.squeeze(exp(rot_angle / 2 * rot_axis.normalize))
    return rot_axis
