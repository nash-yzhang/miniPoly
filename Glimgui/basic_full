...
def prepare():
    self.arduino_board = Arduino('COM24')
...
def set_imgui_widgets():
    ...
    fps_max = 45.
    x_offset = 50.
    self.analog_array.append((len(self.analog_array),self.arduino_board.analog[0].read()))
    ...
    imgui.same_line()
    imgui.text("dt = %.4f"%self.dt)

    _, self.cam_exposuretime = imgui.slider_int('exposure time', self.cam_exposuretime, -13, -1, '%d')
    self.cam.set(cv2.CAP_PROP_EXPOSURE, self.cam_exposuretime)
    ...
    ai_adapted = np.array(self.analog_array)[line_st:]
    ai_adapted[:,1] = (1-(ai_adapted[:,1]-0.)/(1.1-0.))*wh
    ai_adapted += winPos+np.array([-min(ai_adapted[:,0])+x_offset,0.])
    ...
    draw_list.add_polyline(dtlist_adapted.tolist(), imgui.get_color_u32_rgba(0., .5, 1, 1), closed=False,
                           thickness=3)
    draw_list.add_polyline(ai_adapted.tolist(), imgui.get_color_u32_rgba(.5, .5, 1, 1), closed=False,
                           thickness=3)
