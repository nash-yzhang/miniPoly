...
def prepare():
    ...
    radscale = gaussian_filter(np.random.normal(size=[51, 101, 500]), [sp_sigma, sp_sigma / 4, tp_sigma]).reshape(
        [-1, 500])  # Random white noise motion vector
...
def draw(ww, wh):
    ...
    tempV = vecNormalize(self.inputV) * (
        ...
        [np.sin(self.t / 20), np.cos(self.t / 20), 0]) / 5
    temp_azi, temp_elv = cart2sph(tempV[:, 0], tempV[:, 1], tempV[:, 2])
    temp_azi = temp_azi.reshape(-1, 3)
    temp_ptest = (d_range(temp_azi, rangeaxis=1)[:, None] * np.ones([1, 3]) > np.pi) & (temp_azi < 0) & (
                temp_azi > -np.pi)
    temp_azi[temp_ptest] += (np.pi * 2)
    temp_azi = temp_azi.flatten()
    self.V["texcoord"] = np.hstack([temp_azi[:, None] * 40 / 2 / np.pi, temp_elv[:, None]]) / 10 - np.array(
        [self.t, self.t]) * 10 ** -3
