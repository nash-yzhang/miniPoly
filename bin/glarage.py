...
def UVsphere(azi, elv, azitile: int = 30, elvtile: int = 30):
    ...
    # imgV_conn = imgV_conn.reshape(-1, 3)[imgV_conn.max(1) <= len(p3), :]
    # imgV_conn = imgV_conn[imgV_conn.max(1) <= len(p3), :]

    return p3,  imgV_conn.reshape(-1, 3)#[imgV_conn.max(1) <= len(p3), :]
