<h1 align="center">
  <img src="assets/minipoly.png" alt="" width="104"><br>
  miniPoly
</h1>
...
class Follower(AbstractCompiler):
    def on_time(self, t):
        ...
        if reading is not None and reading >= 50:
            ...
            self.shutdown()                                   # a request, not a return
...
Runnable as
[examples/two_minions.py](https://github.com/nash-yzhang/miniPoly/blob/main/examples/two_minions.py).
...
- **Shared ndarray buffers** for what does not belong in a state — camera frames, OpenGL
  framebuffer reads. See [Share frame data](howto/shared-ndarray-buffers.md).
...
same run. Method and provenance:
...
cost is a core per minion — see *Known limitations* below.
...
- **The state namespace is for state, not for bulk data.** One 8 KB JSON segment per
  ...
  buffer, which is a separate, raw path — see [Share frame data](howto/shared-ndarray-buffers.md).
- **The read-write lock is not finished.** Writer-versus-writer exclusion is an atomic
  ...
  **no reader count**, so a writer can still enter while a second reader is inside the
  ...
  [PROJECT_OVERVIEW.md](https://github.com/nash-yzhang/miniPoly/blob/main/docs/PROJECT_OVERVIEW.md)
  4-A4. Genuine multi-reader use is the case to be careful with.
...
- **A crashed minion can still look alive.** A segment outlives its owner, so a process
  ...
  rather than by knowing. See [Detect a crashed minion](howto/detect-a-crashed-minion.md).

## Where to go next

This site, in the order a new reader typically wants them:

- **[Tutorial](tutorial.md)** — run the two-process rig above yourself, then change it.
- **[Architecture](architecture.md)** — how `core` → `compiler` → `processor` → `launcher` fit
  together, generated from the actual import graph and class hierarchy.
- **How-to guides** (left sidebar) — task-oriented recipes: writing a new device
  compiler, choosing between state kinds, detecting a crashed minion, shutting a rig
  down cleanly, and more.
- **Reference** (left sidebar) — every public class and method, generated straight from
  the source's docstrings.

One further document lives in the repository rather than on this site, since it is
maintainer-facing (a graded defect inventory and roadmap) rather than reader-facing:
[PROJECT_OVERVIEW.md](https://github.com/nash-yzhang/miniPoly/blob/main/docs/PROJECT_OVERVIEW.md).
Read the design-intent note in PROJECT_OVERVIEW.md section 2.2 before changing anything
under `miniPoly/core/`.
